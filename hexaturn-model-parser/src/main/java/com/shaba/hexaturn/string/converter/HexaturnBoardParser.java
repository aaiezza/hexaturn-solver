...
import static com.shaba.hexaturn.AbstractSatelliteData.BORDER_HEX;
...
public class HexaturnBoardParser {
    ...
    private HexaturnBoardBuilder generateHexaturnBoardBuilder() {
        return HexaturnBoard.builder().width(width).height(height);
    }

    private HexaturnBoard applyConfiguration(final HexaturnBoardBuilder boardBuilder) {
        final Map<Integer, HexSatelliteData> config = converter.get().convertBoardCode(boardCode);

        boardBuilder.data(IntStream.range(0, width * height)
                .mapToObj(hexIndex -> config.getOrDefault(hexIndex, BORDER_HEX)).collect(toList()));
